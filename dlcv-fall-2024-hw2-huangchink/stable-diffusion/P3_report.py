...
def generate_and_save_images(model, sampler, json_path, output_dir, device):
    ...
    # prompts =  'a photo of a <new1> in the style of <new2>'

    prompts =  "A <new1> perched on a park bench with the Taipei 101 behind in the style of <new2>"

    print(f'prompt:{prompts}')
    prompt_output_dir = os.path.join(output_dir)
    os.makedirs(prompt_output_dir, exist_ok=True)

    # 獲取 prompt 的 conditioning (嵌入向量)
    text_embeddings = model.get_learned_conditioning([prompts]).to(device)
    ...
    for i in tqdm(range(3)):
        ...
        output_path = os.path.join(prompt_output_dir, f'{i}.png')
        output_image.save(output_path)


        print(f'Generated 3 images for sourceprompt {prompts}')
...
def main():
    ...
    embed_path = f'./embedding_checkpoint/new1best.pth'
    embed_path2 = f'./embedding_checkpoint/new2best.pth'
